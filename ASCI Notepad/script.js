...
function displayNotes(){
	...
	notesObj.forEach(function(element,index){
		html += `
				<div class="card mx-4 my-2 bg-white text-dark thatsMyNote" style="width: 18rem;">
					<div class="card-body">
						<h6>${element.time}</h6>
						<p class="card-text">${element.text.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
						<button id="${index}" onclick=deleteNote(this.id) class="btn btn-outline-primary">Delete</button>
					</div>
				</div>
			`;
	});
	...
	if(notesObj.length != 0){
		...
		noteEle.innerHTML = '<h1 style="color: grey;">Nothing to display yet</h1>';
	}
